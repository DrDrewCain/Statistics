...
leaps.then.press.plot<-function(xmat0,yvec,ncheck=4,print.ls=F, resid.plot=F)
  {
    ...
    {
      n1<-ceiling(sqrt(2*ncheck)) # some n variable which gets set as ceiling(sqrt(2*ncheck)) where n check is = number of checks. 
    }
    par(mfrow=c(n1,n1)) # creates column based by nth column and rows array of plots.
    ...
    for(i in 1:ncheck)
      ...
      if(print.ls) # Mom?
      {
        ls.print(ls.str0) # dad?
      }
      print(paste("Press=",ls.str0$press)) # Prints press &  Linear fit str0 accesses press 
      parvec<-matwhich[i,] #relevant parameter vector 
      npar<-sum(parvec)  # nth part becomes sum of parameter vector 
      ...
      if(resid.plot) # conditional residual plot if condition is met it's gonna plot although it should be met in all cases.
      {
...
matrix.2ndorder.make<-function(x, only.quad=T)
  {
    ...
    for(i in 1:num.col){
      ...
      if(!only.quad){
        for(j in i:num.col){
          x_0<-cbind(x_0,x[,i]*x[,j]) # recursively add only the terms of degree 2 
